...
| `org.projectlombok:lombok` | (BOM 2.7.18) | Re-managed; compatible (use 1.18.30+ for Java 21) |
...
**Calibration for this service (important caveat):** Virtual threads are an *optional tuning lever*, not a headline win for a small internal CRUD service. The real concurrency ceiling here is the **HikariCP JDBC connection pool** (default 10 connections) — virtual threads do *not* remove that bound, so DB-bound throughput does not "scale to millions." Treat this as a low-cost option to enable and benchmark, not a guaranteed performance gain. (Note: synchronized-block pinning is largely mitigated in JDK 21 and fully addressed in later JDKs.)
...
1. **`config/SecurityConfig.java` (Highest):** Extends `WebSecurityConfigurerAdapter`, which is **removed** in Spring Security 6. The whole class must be rewritten to a `SecurityFilterChain` `@Bean`. The four access rules (`/h2-console/**` permitAll, `/actuator/**` permitAll, `/api/v1/employees/**` authenticated, `anyRequest` authenticated), CSRF disable, STATELESS session policy, HTTP Basic, and `frameOptions().disable()` must all be preserved exactly. Beyond the mechanical rewrite, three Spring Security 6 **default-behavior changes** can silently alter access even when the rules look identical:
   ...
   - `frameOptions().disable()` and parts of the old `HeadersConfigurer` are deprecated in Security 6.1+; prefer `frameOptions(f -> f.sameOrigin())` scoped to the H2 console rather than a global disable.
...
gantt
    ...
    section Preparation
    Review this report           :a1, 2026-06-26, 1d
...
In this project, only three of these families are actually present: `javax.persistence` (Employee), `javax.validation` (controller + 2 DTOs), and `javax.servlet` (GlobalExceptionHandler).
...
|---|---|---|
| Java source files | 5 (of 15) | 16 import + 1 class rewrite |
...
> **Mockito 5 strict-stubs caveat (likely failure):** `MockitoExtension` enforces `STRICT_STUBS`, unlike the lenient `MockitoJUnitRunner`. Stubs in `EmployeeServiceImplTest` that are not exercised on every test path will throw `UnnecessaryStubbingException`. Resolve by removing the unused stub from the shared `@BeforeEach`/test, scoping it to the test that uses it, or — only where lenient behavior is genuinely required — annotating the class with `@MockitoSettings(strictness = Strictness.LENIENT)`. `spring-security-test` MockMvc helpers (`httpBasic`, `@WithMockUser`, CSRF defaults) may also need minor adjustment under Security 6.
...
sequenceDiagram
    ...
    Note over T,DB: OS carrier threads stay small - Virtual threads scale to millions
```
...
**Verification outcome:** VERIFIED on attempt 2. gem-reviewer confidence 0.97 (no critical issues); gem-critic confirmed all challenged assumptions resolved (confidence 0.90). Risk calibration strengthened (Mockito strict-stubs, Spring Security 6 default behavior, Hibernate 6 + H2 2.x) following adversarial review.

---

*Generated by Spring Boot Upgrade Agent*
*Fact-checked: 2026-06-26 — VERIFIED (reviewer 0.97 / critic 0.90, 2 attempts)*
